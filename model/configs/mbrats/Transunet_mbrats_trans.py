...
from model.gen_trans_2d import segmentation_model
...
def get_r50_b16_config():
    ...
    config.classifier = 'seg'
    config.pretrained_path = '/home/maloadba/ssl-seg-eugene/model/configs/3d_mbrats/R50+ViT-B_16.npz'
    config.decoder_channels = (256, 128, 64, 16)
...
class VisionTransformer_encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        #logits = torch.sigmoid(self.segmentation_head(x))
...
class VisionTransformer_decoder_seg(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        super(VisionTransformer_decoder_seg, self).__init__()
        self.num_classes = num_classes
        self.zero_head = zero_head
        self.classifier = config.classifier
        #self.transformer = Transformer(config, img_size, vis)
...
class VisionTransformer_decoder_trans(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        super(VisionTransformer_decoder_trans, self).__init__()
        self.num_classes = num_classes
        self.zero_head = zero_head
        self.classifier = config.classifier
        #self.transformer = Transformer(config, img_size, vis)
    ...
    def forward(self, x, features):
        ...
        #x, attn_weights, features = self.transformer(x)  # (B, n_patch, hidden)
        x = self.decoder(x, features)
        trans = torch.tanh(self.segmentation_head(x))
        return trans 
                        
            
def build_model(lambda_seg=1, 
                lambda_cyc=1, 
                lambda_disc=1,
    ...
    encoder_instance_source = VisionTransformer_encoder(config, img_size=256, num_classes=1)
    encoder_instance_source.load_from(weights=np.load(config.pretrained_path))
    encoder_instance_target = VisionTransformer_encoder(config, img_size=256, num_classes=1)
    encoder_instance_target.load_from(weights=np.load(config.pretrained_path))

    discriminator_kwargs = {
        'input_dim'           : 1,
        'num_channels_list'   : [480//8, 480//4, 480//2, 480],
        'num_scales'          : 3,
    ...
    submodel = {
        'encoder_source'           : encoder_instance_source,
        'encoder_target'           : encoder_instance_target,
        'decoder_residual_target'  :  VisionTransformer_decoder_seg(config, img_size=256, num_classes=1),
        'decoder_residual_source'  : VisionTransformer_decoder_seg(config, img_size=256, num_classes=1),
        'decoder_source'    : VisionTransformer_decoder_trans(config, img_size=256, num_classes=1),
        'decoder_target'  : VisionTransformer_decoder_trans(config, img_size=256, num_classes=1),           
        'disc_T'            : munit_discriminator(**discriminator_kwargs),
        'disc_S'            : munit_discriminator(**discriminator_kwargs)}
    ...
    model = segmentation_model(**submodel,
                               ...
                               lambda_mod_disc=lambda_disc*lambda_scale,
                               lambda_mod_cyc=lambda_cyc*lambda_scale)
