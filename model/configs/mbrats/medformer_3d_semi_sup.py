...
from model.gen_3d_self_training_mbrats_ss import segmentation_model
...
def build_model(lambda_disc=1,
    ...
    encoder = Encoder(in_chan=1, num_classes=1, base_chan=16, map_size=[3,10,4], 
                conv_block='BasicBlock', conv_num=[2,0,0,0, 0,0,2,2], 
                trans_num=[0,2,4,6, 4,2,0,0], chan_num=[32,64,128,256,128,64,32,16],
                num_heads=[1,4,8,10, 8,4,1,1], fusion_depth=2, fusion_dim=256, 
    ...
    decoder_common = Decoder_common(in_chan=1, num_classes=1, base_chan=16, map_size=[3,10,4], 
                conv_block='BasicBlock', conv_num=[2,0,0,0, 0,0,2,2], 
                trans_num=[0,2,4,6, 4,2,0,0], chan_num=[32,64,128,256,128,64,32,16],
                num_heads=[1,4,8,10, 8,4,1,1], fusion_depth=2, fusion_dim=256, 
                fusion_heads=10, expansion=4, attn_drop=0., proj_drop=0., proj_type='depthwise', 
    ...
    decoder_residual = Decoder_residual(in_chan=1, num_classes=1, base_chan=16, map_size=[3,10,4], 
                conv_block='BasicBlock', conv_num=[2,0,0,0, 0,0,2,2], 
                trans_num=[0,2,4,6, 4,2,0,0], chan_num=[32,64,128,256,128,64,32,16],
                num_heads=[1,4,8,10, 8,4,1,1], fusion_depth=2, fusion_dim=256, 
                fusion_heads=10, expansion=4, attn_drop=0., proj_drop=0., proj_type='depthwise', 
    ...
    decoder_seg = Decoder_residual(in_chan=1, num_classes=1, base_chan=16, map_size=[3,10,4], 
                conv_block='BasicBlock', conv_num=[2,0,0,0, 0,0,2,2], 
                trans_num=[0,2,4,6, 4,2,0,0], chan_num=[32,64,128,256,128,64,32,16],
                num_heads=[1,4,8,10, 8,4,1,1], fusion_depth=2, fusion_dim=256, 
                fusion_heads=10, expansion=4, attn_drop=0., proj_drop=0., proj_type='depthwise', 
    ...
    submodel = {
        ...
        'segmenter'         : decoder_seg,
        'disc_TA'            : munit_discriminator(**discriminator_kwargs),
        'disc_TB'            : munit_discriminator(**discriminator_kwargs)}
    ...
    return OrderedDict((
        ('G', model),
        ('D', nn.ModuleList([model.separate_networks['disc_TA'],
                             model.separate_networks['disc_TB']]))
        ))
...
class Decoder_common(nn.Module):
    ...
    def __init__(self, in_chan, num_classes, base_chan=32, map_size=[4,8,8], 
        ...
        self.bottleneck = nn.Conv3d(chan_num[3]*3//4, chan_num[3], kernel_size=1)
    ...
    def forward(self, semantic_m, skips, map_l):
        ...
        semantic_m = self.bottleneck(semantic_m)
...
class Decoder_residual(nn.Module):
    ...
    def __init__(self, in_chan, num_classes, base_chan=32, map_size=[4,8,8], 
        ...
        conv_block = get_block(conv_block)
        norm_fus = get_norm(norm)
        norm = get_norm('ns')
        act = get_act(act)
        
        self.map_fusion = SemanticMapFusion(chan_num[1:4], fusion_dim, fusion_heads, depth=fusion_depth, norm=norm_fus)
        ...
        self.outres = nn.Conv3d(chan_num[7], 1, kernel_size=1)
        self.outseg = nn.Conv3d(chan_num[7], num_classes, kernel_size=1)

    def forward(self, semantic_m, skips, map_l, mode=0):
...
class ConvNormAct(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, kernel_size=3, stride=1, padding=1,
        ...
        super().__init__()
        assert norm in [nn.BatchNorm3d, nn.InstanceNorm3d, LayerNorm, switching_normalization, True, False]
        assert act in [nn.ReLU, nn.ReLU6, nn.GELU, nn.SiLU, True, False]
        
        norm_args = {}
        if norm == switching_normalization:
        ...
        if preact:
            self.norm = norm(in_ch,**norm_args) if norm else nn.Identity()
        else:
            self.norm = norm(out_ch,**norm_args) if norm else nn.Identity()
...
class BidirectionAttentionBlock(nn.Module):
    def __init__(self, feat_dim, map_dim, out_dim, heads, dim_head, norm=nn.BatchNorm3d, act=nn.ReLU,
        ...
        self.norm1 = norm(feat_dim,**norm_args) if norm else nn.Identity() # norm layer for feature map
        self.norm2 = norm(map_dim,**norm_args) if norm else nn.Identity() # norm layer for semantic map
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, dim, out_dim, norm=nn.BatchNorm3d, proj_type='linear', down_scale=[2,2,2], kernel_size=[3,3,3]):
        ...
        self.norm = norm(merged_dim,**norm_args)
...
class switching_normalization(nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        self.norm0 = nn.InstanceNorm3d(*args, **kwargs)
        self.norm1 = nn.InstanceNorm3d(*args, **kwargs)
        self.mode = 0
